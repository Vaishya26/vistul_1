...
modelName = "vgg19"  # we can try: "simpleAE", "convAE", "vgg19"
trainModel = True
parallel = False
...
imgs_train , args_train = read_imgs_dir(dataTrainDir, extensions, parallel=parallel)
print("Reading test images from '{}'...".format(dataTestDir))
imgs_test , args_test = read_imgs_dir(dataTestDir, extensions, parallel=parallel)
...
for i, emb_flatten in enumerate(E_test_flatten):
    ...
    fname_query = args_test[i] #query fname
    imgs_retrieval = [imgs_train[idx] for idx in indices.flatten()] # retrieval images
    fname_retrieval = [args_train[idx] for idx in indices.flatten()] #retrieval fname
    outFile = os.path.join(outDir, "{}_retrieval_{}.png".format(modelName, i))


    comparisions.append({fname_query: fname_retrieval})

    # print(fname_query,fname_retrieval)
